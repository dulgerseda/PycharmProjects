'''
Exercises regarding functions and packing operators.

Author: RCH
Date: 25.11.2024
'''
import random as rd

# given is the following function
def rnd_number(n=1, range=range(1,100), duplications=False):
    '''
    random number generator. This function returns a list of n random numbers being
    within the given range and possible duplicates if duplications is True.
    :param n: number of random numbers to generate
    :param range: numerical range for random numbers
    :param duplications: flag if duplications are allowed
    :return: list of random numbers with or without duplicates
    '''
    rnd_list = []
    if(duplications and n>len(range)):
        return "ERROR: too many elements requested."
    elif(duplications):
        rnd_list = rd.choices(range, k=n)
    else:
        rnd_list = rd.sample(range, k=n)
    return rnd_list

rnd = rnd_number(10, range=range(1,100), duplications=False)
print(rnd)
# Exercise 1:
'''
write a function that returns a list of n random numbers in ascending or descending order.
Call the given function to generate the list and return the sorted list.
'''
import random as rd

def rnd_number(n=1, range=range(1,100), duplications=False):
    '''
    random number generator. This function returns a list of n random numbers being
    within the given range and possible duplicates if duplications is True.
    :param n: number of random numbers to generate
    :param range: numerical range for random numbers
    :param duplications: flag if duplications are allowed
    :return: list of random numbers with or without duplicates
    '''

def listsortfunc(n=10, duplications=False):
    numbers = rnd_number(n, range=range(1,100), duplications=duplications)
    sorted_list = sorted(numbers, reverse=True)
    return sorted_list

rnd_sorted = listsortfunc(10, duplications=False)
print("Sorted list:", rnd_sorted)


# Exercise 2:
'''
write a function that accepts arbitrary number of integer elements. The functions
calculates and returns the average of the difference of all subsequent elements.
Annotate the types to all interface variables more clarity.
'''

def step_avg(*n: int) -> int:
    total = 0
    if len(n) >= 2:
        for i in range(1, len(n)):
            total += n[i] - n[i-1]
        return total / (len(n)-1)
    else:
        return 0

print(step_avg(3, 9, 12, 20))   # farklar = [6, 3, 8] → ortalama = 5.666...

print(step_avg(7))    # sonuç = 0.0

print(step_avg())   # sonuç = 0.0

# Exercise 3:
'''
create two lists containing 10 sub-lists of random numbers.
Each of the two contains 10 sub-lists with n=10,11,12,13,14...19 elements.
One list is sorted in ascending order and the other in descending order.
'''

def listsortfunc(n=10, duplications=False, descending=False):
    numbers = rnd_number(n, range=range(1,100), duplications=duplications)
    sorted_list = sorted(numbers, reverse=descending)
    return sorted_list

l0 = []  # ascending listeler
l1 = []  # descending listeler

for i in range(10, 20):
    l0.append(listsortfunc(i, duplications=False, descending=False))  # küçükten büyüğe
    l1.append(listsortfunc(i, duplications=False, descending=True))   # büyükten küçüğe

print("Ascending lists (l0):")
for sublist in l0:
    print(sublist)

print("\nDescending lists (l1):")
for sublist in l1:
    print(sublist)
"""
Descending lists (l1):
[96, 84, 75, 67, 53, 47, 34, 32, 10, 8]
[80, 74, 73, 60, 58, 50, 32, 21, 16, 13, 4]
[97, 88, 73, 58, 45, 37, 33, 29, 19, 14, 9, 8]
[93, 86, 84, 82, 66, 45, 43, 39, 31, 24, 15, 9, 8]
[96, 88, 86, 80, 76, 68, 54, 49, 46, 41, 38, 33, 6, 4]
[81, 74, 69, 67, 63, 60, 58, 55, 50, 45, 42, 26, 12, 10, 6]
[98, 93, 91, 80, 76, 68, 67, 58, 55, 53, 31, 27, 14, 10, 6, 5]
[94, 93, 89, 80, 78, 73, 66, 65, 63, 61, 43, 37, 34, 16, 10, 8, 1]
[97, 96, 95, 88, 68, 67, 64, 59, 55, 45, 44, 42, 25, 19, 16, 10, 8, 3]
[93, 85, 70, 65, 57, 54, 52, 51, 46, 45, 43, 30, 29, 21, 12, 8, 7, 5, 1]"""


# Exercise 4:
'''
calculate the mean of the average element distance among all ascending and descending 
sorted lists. Use the function from Exercise 2.
Are the two absolute averages comparable?
'''

def step_avg(*n: int) -> int:
    total = 0
    if len(n) >= 2:
        for i in range(1, len(n)):
            total += n[i] - n[i-1]
        return total / (len(n)-1)
    else:
        return 0


avg0 = avg1 = 0
for i in range(len(l0)):
    avg0 += step_avg(*l0[i])/len(l0)
    avg1 += step_avg(*l1[i])/len(l1)

print(f'{avg0=}', f'{avg1=}')